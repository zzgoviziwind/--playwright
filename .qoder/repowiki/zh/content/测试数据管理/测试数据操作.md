...
## 项目结构
该仓库采用端到端测试工程组织方式，围绕 Playwright 进行页面行为测试，同时提供 API 与数据库工具用于数据准备与清理。关键目录与文件如下：
...
- 夹具（Fixtures）
  ...
  - 数据夹具：自动创建与清理测试报告，支持草稿、待审核、已审核三类状态
- 工具（Utils）
...
- 批量数据创建
  ...
  - 示例路径：[createTestReport:83-121](file://e2e-tests/utils/api-helper.ts#L83-L121)
- 随机数据生成
...
- 夹具自动创建与清理
  - 数据夹具在测试生命周期内自动创建/删除报告，确保环境干净
  - 示例路径：[data.fixture.ts:1-57](file://e2e-tests/fixtures/data.fixture.ts#L1-L57)
...
### 数据清理机制
- 测试结束自动清理
  - 夹具在 use 生命周期结束后调用删除接口，失败不阻断测试
  - 示例路径：[data.fixture.ts:14-53](file://e2e-tests/fixtures/data.fixture.ts#L14-L53)
- 全局清理
  - teardown 清理 storageState 文件，避免跨测试污染
  - 示例路径：[auth.teardown.ts:1-18](file://e2e-tests/fixtures/auth.teardown.ts#L1-L18)
- 批量删除与状态重置
  - API 提供按前缀批量清理接口
  - 数据库提供按前缀删除与重置函数
  - 示例路径：[api-helper.ts:155-161](file://e2e-tests/utils/api-helper.ts#L155-L161)、[db-helper.ts:33-43](file://e2e-tests/utils/db-helper.ts#L33-L43)
- 环境清理
...
- 数据保护
  ...
  - 示例路径：[db-helper.ts:35-42](file://e2e-tests/utils/db-helper.ts#L35-L42)
- 登录态隔离
  - 不同角色使用独立 storageState 文件，避免会话串扰
  - 示例路径：[auth.fixture.ts:10-37](file://e2e-tests/fixtures/auth.fixture.ts#L10-L37)

章节来源
- [playwright.config.ts:24-29](file://e2e-tests/playwright.config.ts#L24-L29)
- [Jenkinsfile:8-10](file://e2e-tests/Jenkinsfile#L8-L10)
- [.gitlab-ci.yml:8-9](file://e2e-tests/.gitlab-ci.yml#L8-L9)
- [db-helper.ts:35-42](file://e2e-tests/utils/db-helper.ts#L35-L42)
- [auth.fixture.ts:10-37](file://e2e-tests/fixtures/auth.fixture.ts#L10-L37)

### 数据维护流程
- 数据更新
  - 通过 API 工具直接更新报告状态，用于准备前置数据
  - 示例路径：[api-helper.ts:134-142](file://e2e-tests/utils/api-helper.ts#L134-L142)
- 版本管理与变更追踪
...
- 并发与一致性
  ...
  - 示例路径：[api-helper.ts:86-87](file://e2e-tests/utils/api-helper.ts#L86-L87)
...
- [api-helper.ts:86-87](file://e2e-tests/utils/api-helper.ts#L86-L87)
...
- 同步方法
  ...
  - 示例路径：[report-list.page.ts:46-59](file://e2e-tests/pages/report-list.page.ts#L46-L59)
...
### API 辅助工具与数据库工具使用示例
- API 辅助工具
  - 获取 API 上下文与认证：[getApiContext:45-77](file://e2e-tests/utils/api-helper.ts#L45-L77)
  ...
  - 查询报告详情：[getReport:147-151](file://e2e-tests/utils/api-helper.ts#L147-L151)
  - 批量清理：[cleanupTestReports:155-161](file://e2e-tests/utils/api-helper.ts#L155-L161)
  - 销毁上下文：[disposeApiContext:166-171](file://e2e-tests/utils/api-helper.ts#L166-L171)
- 数据库辅助工具
  ...
  - 按前缀清理：[cleanupReportsByPrefix:48-54](file://e2e-tests/utils/db-helper.ts#L48-L54)
  - 从数据库验证状态：[getReportStatusFromDB:59-67](file://e2e-tests/utils/db-helper.ts#L59-L67)
...
章节来源
- [api-helper.ts:45-171](file://e2e-tests/utils/api-helper.ts#L45-L171)
- [db-helper.ts:11-90](file://e2e-tests/utils/db-helper.ts#L11-L90)

### 页面对象与测试用例中的数据操作
- 页面对象
  - 报告列表页封装了搜索、筛选、分页、打开/编辑/删除等交互，内部等待 API 响应
  - 示例路径：[report-list.page.ts:34-129](file://e2e-tests/pages/report-list.page.ts#L34-L129)
- 测试用例
  - 冒烟测试：验证列表加载与关键列
...
- 并发与资源限制
  ...
  - 示例路径：[api-helper.ts:86-87](file://e2e-tests/utils/api-helper.ts#L86-L87)、[db-helper.ts:20-22](file://e2e-tests/utils/db-helper.ts#L20-L22)
- 等待策略
  - 使用等待工具减少竞态条件，提升稳定性
  - 示例路径：[wait-helper.ts:8-23](file://e2e-tests/utils/wait-helper.ts#L8-L23)、[report-list.page.ts:46-59](file://e2e-tests/pages/report-list.page.ts#L46-L59)
- CI 并行
...
## 结论
本仓库提供了完整的测试数据操作闭环：从数据生成、状态准备、执行验证到清理与归档，配合 API 与数据库工具实现高效、稳定、可追溯的测试数据管理。通过环境隔离、前缀策略与 CI 归档，确保测试过程可控、可重复、可审计。建议在团队内推广使用夹具与工具函数，统一标准流程，持续优化等待与重试策略，保障大规模并发场景下的稳定性。
