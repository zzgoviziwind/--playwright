...
## 更新摘要
**变更内容**
- 优化了表格加载等待策略，增加了更精确的加载状态检测
- 改进了API响应等待机制，支持更灵活的URL模式匹配
- 增强了Toast提示等待的可靠性，提供更好的用户体验
- 优化了重试包装器，增加了更智能的错误处理机制
- 完善了等待策略的超时处理，提供了更精确的异常管理
...
**更新** 等待辅助工具已优化，提供了更精确的等待策略和超时处理机制，增强了对复杂场景的适配能力和错误恢复能力。
...
RD["report-detail.page.ts<br/>详情页对象"]
NAV["navigation.page.ts<br/>导航页对象"]
end
subgraph "测试层"
...
## 核心组件
- **表格加载完成等待**：基于容器可见、加载遮罩隐藏、首行数据可见的多阶段等待，支持更精确的加载状态检测。
- **Toast/消息提示等待**：基于文本可见的显式等待，提供更好的用户体验和错误恢复机制。
- **API 响应等待**：基于响应 URL 匹配与状态码过滤的条件等待，支持灵活的URL模式匹配。
- **SPA 导航完成等待**：基于网络空闲的页面加载状态等待，确保路由切换的稳定性。
- **操作重试包装器**：对不稳定操作进行有限次数的延迟重试，提供智能的错误处理机制。
- **文本内容等待**：基于过滤器匹配的元素可见等待，支持更精确的内容匹配。
- **默认超时常量**：统一的默认超时值，便于全局一致性管理。
...
### 同步等待策略
- **表格加载完成等待**：先等待表格容器可见，再尝试等待加载遮罩隐藏（若存在），最后等待首行数据可见。该策略通过"可见性 + 可见性"保证 DOM 结构稳定与数据呈现，支持多种加载指示器类型。
...
### 异步等待策略
- **API 响应等待**：基于 URL 模式匹配与状态码过滤，等待特定接口返回。该策略支持字符串和正则表达式两种URL匹配方式，避免盲目 sleep，精准等待网络请求完成。
...
### 条件等待策略
- **Toast/消息提示等待**：基于文本可见的条件等待，确保用户反馈及时出现，支持多种消息类型的识别。
...
- [report-crud.spec.ts:8-27](file://e2e-tests/tests/regression/report-crud.spec.ts#L8-L27)
...
WH --> RD["report-detail.page.ts"]
WH --> NAV["navigation.page.ts"]
...
- **智能超时处理**：通过 catch 机制优雅处理可选等待，避免不必要的异常中断。

**更新** 优化了超时处理机制，提供了更智能的异常恢复策略，提高了整体测试执行效率。
...
- **加载指示器问题**：支持多种加载指示器类型，包括骨架屏和加载遮罩，提高兼容性。

**更新** 增强了故障排查能力，提供了更详细的错误诊断和恢复机制。
...
## 结论
等待辅助工具通过"显式 + 条件 + 重试"的策略组合，有效提升了端到端测试的稳定性与可维护性。配合 Playwright 的原生等待能力与合理的超时配置，能够在保证质量的同时兼顾执行效率。

**更新** 优化后的等待辅助工具提供了更精确的等待策略和超时处理机制，显著提升了复杂场景下的适配能力和错误恢复能力，为自动化测试提供了更可靠的基础设施支持。
