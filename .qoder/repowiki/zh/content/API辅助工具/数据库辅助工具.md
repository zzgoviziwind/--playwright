...
## 更新摘要
**所做更改**
- 更新了连接池配置和生命周期管理章节，反映最新的单例连接池实现
- 新增了测试数据清理功能的详细说明
- 增强了查询封装和SQL注入防护的分析
- 补充了数据库操作的最佳实践和性能优化策略
- 更新了故障排查指南，包含新的常见问题
...
- auth.setup.ts/auth.teardown.ts：认证状态管理与清理
...
SETUP["全局 Setup/Cleanup<br/>auth.setup.ts/auth.teardown.ts"]
end
subgraph "工具层"
...
**更新** 增强了测试数据清理功能，提供了更完善的测试数据管理策略

- 重置测试数据：支持清理所有以测试前缀命名的报告数据
- 按前缀清理：支持按任意前缀批量删除测试数据
- 数据一致性：与 API 辅助工具配合，确保测试数据的完整性和一致性

```mermaid
flowchart TD
Reset["重置测试数据"] --> Prefix1["清理冒烟测试数据"]
Prefix1 --> Prefix2["清理回归测试数据"]
Prefix2 --> IdPrefix["清理 TEST* ID 数据"]
IdPrefix --> Complete["测试数据重置完成"]
Cleanup["按前缀清理"] --> Input["输入清理前缀"]
Input --> Delete["执行 DELETE 操作"]
Delete --> Verify["验证清理结果"]
Verify --> Success["清理完成"]
```
...
- 测试数据污染
  - 检查测试数据清理函数是否正确执行
  - 确认测试前缀使用的一致性
  - 参考路径：[db-helper.ts:33-43](file://e2e-tests/utils/db-helper.ts#L33-L43)，[db-helper.ts:48-54](file://e2e-tests/utils/db-helper.ts#L48-L54)
