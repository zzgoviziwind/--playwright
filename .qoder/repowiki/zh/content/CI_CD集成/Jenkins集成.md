...
## 更新摘要
**所做更改**
- 更新了容器化测试执行环境的描述，强调了Microsoft Playwright官方镜像的使用
- 增强了Docker Agent配置的说明，突出容器化环境的一致性和稳定性
- 补充了GitLab CI与Jenkins集成的最佳实践对比
- 完善了容器化环境下的故障排查指南
...
## 简介
本指南面向Jenkins管理员与测试工程师，提供基于Playwright的端到端测试在Jenkins中的完整实施方法。经过最新的容器化改进，系统现在使用Microsoft官方Playwright镜像，确保测试环境的稳定性和一致性。内容涵盖：
...
## 核心组件
- **Jenkins声明式流水线**
  - 使用Microsoft官方Playwright Docker镜像，确保跨平台一致性与稳定性
  - 环境变量BASE_URL指向测试服务器
...
## 架构总览
Jenkins流水线在Docker容器中执行，使用Microsoft官方Playwright镜像确保环境一致性，按阶段顺序运行安装、冒烟与回归测试，并在后置阶段发布报告与归档制品。Playwright根据配置选择项目并并行执行，生成多格式报告与测试结果。

```mermaid
sequenceDiagram
participant J as "Jenkins"
participant D as "Docker Agent<br/>Microsoft Playwright v1.50.0"
...
### Jenkinsfile声明式流水线
- **执行模型**
  - **Agent**：使用Microsoft官方Playwright镜像`mcr.microsoft.com/playwright:v1.50.0-jammy`，确保跨平台一致性与稳定性
...
```mermaid
flowchart TD
S["开始"] --> A["Agent Docker 镜像<br/>Microsoft Playwright v1.50.0"]
A --> E["设置环境变量 BASE_URL"]
E --> I["Install 阶段<br/>pnpm install --frozen-lockfile"]
...
**图表来源**
- [playwright.config.ts:6-52](file://e2e-tests/playwright.config.ts#L6-L52)
...
- **容器化环境优势**
  - Microsoft官方Playwright镜像确保依赖一致性，避免运行时差异导致的不稳定
  - pnpm安装锁定版本，确保依赖一致性
- **报告与制品**
...
## 故障排查指南
- **容器化环境问题**
  - **镜像拉取失败**：检查网络连接和镜像仓库访问权限
  - **容器启动异常**：确认Docker守护进程状态和资源配额
  - **权限不足**：确保Jenkins用户具有docker执行权限
- **浏览器启动失败**
  - 检查Microsoft Playwright镜像是否包含必要依赖；确认DISPLAY或headless配置
...
**章节来源**
- [auth.setup.ts:1-116](file://e2e-tests/fixtures/auth.setup.ts#L1-L116)
- [api-helper.ts:200-206](file://e2e-tests/utils/api-helper.ts#L200-L206)
- [Jenkinsfile:51-57](file://e2e-tests/Jenkinsfile#L51-L57)

## 结论
本指南提供了从Jenkins流水线到Playwright测试的完整实施路径。通过使用Microsoft官方Playwright镜像的容器化环境，系统获得了更高的稳定性与一致性。通过明确的阶段划分、稳定的执行环境、完善的报告与制品归档，以及可扩展的通知机制，能够有效支撑持续集成与交付。建议在生产环境中进一步完善参数化配置、负载均衡与性能监控，以获得更稳健的自动化测试体系。
...
### 构建环境设置
- **Docker镜像**
  - 使用Microsoft官方Playwright镜像`mcr.microsoft.com/playwright:v1.50.0-jammy`，确保Node与浏览器版本一致
- **Node与包管理**
  - 使用pnpm并锁定版本，避免依赖漂移
- **Playwright配置**
...
**章节来源**
- [Jenkinsfile:2-6](file://e2e-tests/Jenkinsfile#L2-L6)
...
### GitLab CI与Jenkins集成对比
- **相似点**
  - 都使用Microsoft官方Playwright镜像
  - 都采用容器化测试执行环境
  - 都支持多项目并行执行
- **差异点**
  - Jenkins使用声明式流水线，GitLab使用YAML配置
  - Jenkins支持更丰富的插件生态，GitLab集成度更高
  - Jenkins更适合复杂的CI/CD流程，GitLab更适合DevOps一体化

**章节来源**
- [.gitlab-ci.yml:14](file://e2e-tests/.gitlab-ci.yml#L14)
- [Jenkinsfile:4](file://e2e-tests/Jenkinsfile#L4)